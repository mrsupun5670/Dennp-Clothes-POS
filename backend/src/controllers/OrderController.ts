...
class OrderController {
  ...
  async createOrder(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (items && Array.isArray(items)) {
        ...
        // Reduce stock quantities for each item - DIRECT UPDATE QUERY
        logger.info("=== STARTING STOCK REDUCTION ===", {
          orderId,
          shopId: shop_id,
        ...
        for (const item of items) {
          if (item.product_id && item.size_id && item.color_id && item.quantity) {
            try {
              // Ensure product_id is a string
              const productIdStr = String(item.product_id);

              logger.info("Reducing stock for item", {
              ...
              // Direct UPDATE query to shop_product_stock table
              ...
              const updateParams = [item.quantity, shop_id, productIdStr, item.size_id, item.color_id];
              ...
              logger.info("Stock UPDATE result", {
                productId: productIdStr,
                sizeId: item.size_id,
                colorId: item.color_id,
                affectedRows,
            ...
            logger.warn("Skipping stock reduction - missing fields", {
              item,
            });
          }
        }

        logger.info("=== STOCK REDUCTION COMPLETED ===", {
          orderId,
          processedItems: items.length,
        });
      }

      // Update customer statistics (orders_count and total_spent)
      if (customer_id) {
        try {
          await CustomerModel.updateCustomerStats(customer_id, shop_id);
          logger.info("Customer stats updated after order creation", {
            customerId: customer_id,
            orderId,
          });
        } catch (customerError) {
          logger.error("Error updating customer stats:", {
            customerId: customer_id,
            orderId,
            error: customerError,
          });
  ...
  /**
   * GET /orders/generate-number - Generate next order number for a shop
   */
  async generateOrderNumber(req: Request, res: Response): Promise<void> {
    try {
      const shopId = Number(req.query.shop_id);
      ...
      const orderNumber = await OrderModel.generateOrderNumber(shopId);
      ...
      logger.error("Error in generateOrderNumber:", error);
      res.status(500).json({
        success: false,
  ...
  async recordPayment(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Recalculate payment status to update balance_due and payment_status
      // This ensures the database reflects the current state including delivery charges
