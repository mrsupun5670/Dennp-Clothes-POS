...
ALTER TABLE orders ADD COLUMN advance_paid DOUBLE DEFAULT 0 AFTER total_amount COMMENT 'Amount paid as advance or partial payment';

-- Step 2: Add balance_paid column to track final/remaining payments
ALTER TABLE orders ADD COLUMN balance_paid DOUBLE DEFAULT 0 AFTER advance_paid COMMENT 'Amount paid as balance payment';

-- Step 3: Add total_paid column for quick reference (advance_paid + balance_paid)
ALTER TABLE orders ADD COLUMN total_paid DOUBLE DEFAULT 0 AFTER balance_paid COMMENT 'Total amount paid (advance + balance)';

-- Step 4: Add payment_status to track overall payment state
ALTER TABLE orders ADD COLUMN payment_status ENUM('unpaid', 'partial', 'fully_paid') DEFAULT 'unpaid' AFTER total_paid COMMENT 'Current payment status of order';

-- Step 5: Add remaining_amount column for quick calculation (total_amount - total_paid)
ALTER TABLE orders ADD COLUMN remaining_amount DOUBLE DEFAULT 0 AFTER payment_status COMMENT 'Remaining amount to be paid';

-- Step 6: Add indexes for payment tracking queries
ALTER TABLE orders ADD INDEX idx_payment_status (payment_status);
ALTER TABLE orders ADD INDEX idx_total_paid (total_paid);
