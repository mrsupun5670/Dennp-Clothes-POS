// COMPLETE FIX: printBill for OrdersPage
// This fetches UNGROUPED items (with size/color) for printing
// Replace the entire printBill function (lines 373-471) with this:

const printBill = async (order: Order) => {
  try {
    // Fetch UNGROUPED order items for printing (with size/color data)
    const itemsResponse = await fetch(`${API_URL}/orders/${order.order_id}/items-ungrouped`);
    
    if (!itemsResponse.ok) {
      alert("❌ Could not fetch order items. Cannot print bill.");
      return;
    }
    ...
    setTimeout(async () => {
      try {
        ...
        const tempFileName = `print-temp-${order.order_number}-${Date.now()}.png`;
...
// KEY CHANGE: Uses /items-ungrouped endpoint to get size/color details
// This endpoint should return items like:
// [
//   { product_name: "T-Shirt", size_name: "L", color_name: "Black", quantity: 2, ... },
//   { product_name: "T-Shirt", size_name: "M", color_name: "Red", quantity: 1, ... }
// ]
//
// If this endpoint doesn't exist, you need to create it in the backend!
